...
export class FeedbackCreateComponent implements OnInit {
  ...
  addTopic(){
    if(this.fileToUpload !== null) {
      this.upload();
      this.imagName= this.fileToUpload.name;
    }
    debugger;
    this._http.createTopic({controllerName:this.route.snapshot.params['id'],topicName:this.topicName,topicDetails:this.topicDetails,imageUrl: this.imagName}).subscribe(
      data=>{
        debugger;
      },
      error=>{
